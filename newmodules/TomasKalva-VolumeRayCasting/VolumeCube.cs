...
namespace TomasKalva
{
  ...
  public class VolumeCube : Cube, ISolid, ITimeDependent
  {
    ...
    public static Func<Vector3d, double> ParaboloidShape (Vector3d vertex, Vector3d scale) 
    {
      return v =>
      {
    ...
    /// <param name="vertex">Peak of the paraboloid</param>
    /// <param name="scale">Scale of the inputs</param>
    public static Func<Vector3d, double> ParaboloidFireShape (Vector3d vertex, Vector3d scale)
    {
      Func<Vector3d, double> paraboloid = ParaboloidShape(vertex, scale);
      Func<Vector3d, double> fadeBottom = (v) => v.Y < 0.2 ? v.Y / 0.2 : 1;
      Func<Vector3d, double> fadeTop = (v) => (1 - v.Y);
      return v =>
      {
    ...
    /// <param name="scale">Scale of the inputs</param>
    public static Func<Vector3d, double> BallShape(Vector3d center, Vector3d scale)
    {
      return v =>
      {
    ...
    /// <summary>
    /// Creates function returning color of fire.
    /// </summary>
    ...
    public static Func<Vector3d, double, Vector3d> Fire (
      Func<Vector3d, double> shape,
      Func<Vector3d, double> textureNoise,
      Func<Vector3d, double> displNoise,
      ...
      double intensityMult = 1.0)
    {
      return (Vector3d v, double t) =>
      {
        ...
        var intensity = intensityMult * textureNoise(scaledV + offset + displ) * shape(v + 0.2 * displ) * ( v.Y < 0.1 ? 0 : 1);
